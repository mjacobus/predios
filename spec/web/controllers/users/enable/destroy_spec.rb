...
RSpec.describe Web::Controllers::Users::Enable::Destroy, type: :action do
  ...
  let(:the_user) { User.new(name: 'the-name').enable }
  ...
  context 'with a valid user' do
    ...
    it 'saves user' do
      ...
      expect(repository).to have_received(:save) do |received|
        expect(received).to be_equal_to(the_user.disable)
      end
    end
  end
