  ...
  def initialize
    ...
    share('es.projection_events') do |_container|
      Koine::EventManager::EventManager.new.tap do |manager|
        manager.attach_listener(Buildings::BuildingsProjectionsListener.new)
        manager.attach_listener(Apartments::ApartmentsProjectionsListener.new)
      end
    end
    ...
    add_command_handler('buildings.create_building') do |container|
      Buildings::CommandHandlers::CreateBuilding.new(
        ...
        validator: Buildings::CommandValidators::CreateBuildingValidator.new(
    ...
    add_command_handler('buildings.delete_building') do |container|
      Buildings::CommandHandlers::DeleteBuilding.new(
        repository: container.repository('aggregate_root')
      )
    end
    ...
    add_command_handler('apartments.unassign_contact_attempt') do |container|
      Apartments::CommandHandlers::UnassignContactAttempt.new(
        repository: container.repository('aggregate_root'),
        validator: NullValidator.new
      )
    end
